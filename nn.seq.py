#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2023/9/10 22:43
# @Author  : LingJiaXiaoHu
# @File    : nn.seq.py
# @Software: win11 pytorch(GPU版本） python3.9.16
import torch
from torch import nn
from torch.nn import Conv2d, MaxPool2d, Flatten, Linear, Sequential
from torch.utils.tensorboard import SummaryWriter


class Tudui(nn.Module):
    def __init__(self):
        super(Tudui,self).__init__()
        #跟着CIFAR10模型结构图走   Conv2d(in_channels=,out_channels=,kernel_size=,stride=,padding=)
        #输出大小N = (输入大小W - 卷积核大小F + 2倍填充值大小P)/步长大小S + 1
        # self.conv1 = Conv2d(3,32,5,padding=2)  #卷积
        # self.maxpool1 = MaxPool2d(2)    #池化
        # self.conv2 = Conv2d(32,32,5,padding=2)
        # self.maxpool2 = MaxPool2d(2)
        # self.conv3 = Conv2d(32,64,5,padding=2)
        # self.maxpool3 = MaxPool2d(2)
        # self.flatten = Flatten()  #展平成一排
        # self.linear1 = Linear(1024,64)  #线性变换
        # self.linear2 = Linear(64,10)

        self.model1 = Sequential(
            Conv2d(3, 32, 5, padding=2),
            MaxPool2d(2),
            Conv2d(32, 32, 5, padding=2),
            MaxPool2d(2),
            Conv2d(32, 64, 5, padding=2),
            MaxPool2d(2),
            Flatten(),
            Linear(1024, 64),
            Linear(64, 10)
        )

    def forward(self,x):
        # x = self.conv1(x)
        # x = self.maxpool1(x)
        # x = self.conv2(x)
        # x = self.maxpool2(x)
        # x = self.conv3(x)
        # x = self.maxpool3(x)
        # x = self.flatten(x)
        # x = self.linear1(x)
        # x = self.linear2(x)

        x = self.model1(x)          #使用Squential()简洁很多
        return x

tudui = Tudui()
print(tudui)
# 自行创建 input
input = torch.ones((64,3,32,32))
output = tudui(input)
print(output.shape)

writer = SummaryWriter("./logs_sqe")
writer.add_graph(tudui,input)
writer.close()



